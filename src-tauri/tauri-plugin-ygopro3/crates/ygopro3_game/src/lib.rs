...
impl Game {
	...
	pub async fn init (app: &AppHandle, overwrite: bool) -> Result<Self, Error> {
		...
		let (system, resource, lflist, servers, room, setcode, mut tasks) = Self::load_config(path, &config).await;
		progress::emit(app, Event::Progress, 1);
	...
	async fn load_config (path: &Path, config: &Vec<(String, String)>) -> (System, Resource, LFList, Server, Room, SetCode, Vec<JoinHandle<Result<(), Error>>>) {
	...
	pub async fn get_room () -> Result<Vec<(String, String)>, Error> {
		let game: &RwLock<Self> = GAME.get().ok_or(anyhow!(""))?;
		let game: RwLockReadGuard<'_, Self> = game.read().await;
		Ok(game
			.room
	...
	pub async fn download (app: &AppHandle, url: String, name: String, chunk: usize) -> Result<String, Error> {
		...
		download(app, path.join("expansions"), &url, &name, chunk, max_retries).await
	}
